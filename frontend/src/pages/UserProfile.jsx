...
const Card = ({ children, className }) => <div className={`bg-white shadow-xl rounded-xl p-6 ${className}`}>{children}</div>;
const CardHeader = ({ children, className }) => <div className={`border-b pb-4 mb-4 ${className}`}>{children}</div>;
const CardTitle = ({ children, className }) => <h2 className={`text-2xl font-semibold text-gray-800 ${className}`}>{children}</h2>;
...
const AllTeachersModal = ({ isOpen, onClose, teachers, currentUserId }) => {
    const otherUsers = teachers.filter(user => user.user_profile_id !== currentUserId);

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Danh sách Giảng viên/Tất cả Người dùng">
            <p className="text-gray-700 mb-4">Danh sách tất cả hồ sơ người dùng khác:</p>
            
            <div className="max-h-80 overflow-y-auto border rounded-lg p-2 bg-gray-50">
                {otherUsers.length > 0 ? (
                    <ul className="space-y-3">
                        {otherUsers.map((user, index) => (
                            <li key={user.user_profile_id} className="p-3 border-b last:border-b-0 bg-white rounded-md shadow-sm flex justify-between items-center text-sm">
                                <div>
                                    <p className="font-semibold text-gray-800">{`${user.user_firstname} ${user.user_lastname}`}</p>
                                    <p className="text-xs text-gray-500">{user.user?.user_email || 'Không có Email'}</p>
                                </div>
                    ...
                    <p className="text-center text-gray-500 p-4">Không tìm thấy giảng viên hoặc người dùng nào khác.</p>
                )}
            </div>
...
export default function UserProfile() {
    ...
    const profile = React.useMemo(() => {
        if (allProfiles && Array.isArray(allProfiles)) {
            return allProfiles.find(p => p.user_profile_id === CURRENT_USER_PROFILE_ID); 
        }
        return null;
    }, [allProfiles]);
    ...
    if (error || !profile) {
        const errorMessage = error ? error : `Không tìm thấy hồ sơ người dùng có ID: ${CURRENT_USER_PROFILE_ID}`;
        return (
            <div className="p-8 text-center text-red-600 bg-red-50 border border-red-200 rounded-lg">
                ...
                <p>{errorMessage}</p>
                <p className="text-sm mt-2">Vui lòng kiểm tra console hoặc đảm bảo server API đang chạy tại `{API_URL_ALL}` và trả về hồ sơ có ID {CURRENT_USER_PROFILE_ID}.</p>
            </div>
        );
    }

    const fullName = `${profile.user_firstname || ''} ${profile.user_lastname || ''}`.trim() || 'Người dùng không tên';

    const isCurrentUserAdmin = profile.category_user_type?.user_type_code === 'ADMIN';
    ...
    return (
        <div className="p-6 md:p-10 bg-gray-50 min-h-screen">
            <div className="flex justify-between items-center mb-8 border-b pb-4">
                <h1 className="text-4xl font-extrabold text-gray-900">Hồ Sơ Người Dùng</h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-1 bg-indigo-50 border border-indigo-200">
                    <div className="flex flex-col items-center">
                        ...
                        />
                        <CardTitle className="text-center mb-1 text-indigo-800">{fullName}</CardTitle>
                        <p className="text-sm font-light text-indigo-600 mb-4">{profile.user?.user_email || 'Email không có'}</p>
                        <div className="inline-block bg-indigo-200 text-indigo-900 px-3 py-1 rounded-full text-xs font-bold">
                            {profile.category_user_type?.user_type_name || 'Loại người dùng không xác định'}
                        </div>
                    </div>
                </Card>
            ...
            <Modal isOpen={isEditing} onClose={handleCloseEdit} title="Chỉnh sửa Hồ sơ Cá nhân">
                <form onSubmit={handleFormSubmit} className="space-y-4">
                    ...
                    <div className="flex flex-col items-center space-y-3 p-4 border rounded-lg bg-gray-50">
                        ...
                        />
                        <p className="text-xs text-gray-500 text-center">Chọn file ảnh (JPEG, PNG, JPG, GIF). Dung lượng tối đa 2MB.</p>
                    </div>
                    ...
                    <div className="flex justify-end space-x-3 pt-4">
                        ...
                        <Button
                            type="submit"
                            className={`text-white shadow-md ${isSaving ? 'bg-green-400' : 'bg-green-600 hover:bg-green-700'}`}
                            disabled={isSaving}
                        >
